# -*- coding: utf-8 -*-
"""
Created on Sat Mar 22 12:26:05 2014
...
@author: KyoungWon
"""
#def sc_CdSe(length_sweep, Vm_sweep):
...
for i in range(30):
    ...
    Velectron=Velectron*e/4/pi/eo/er1
    Vhole=Vhole*e/4/pi/eo/er1
    ...
    kkp[0:n, 0:n]=-Ae/m/e*(hbar**2)/2/(space**2) + Pe_grad - Vhole_mat
    kkp[n:2*n, n:2*n]= Ah/m/e*(hbar**2)/2/(space**2) - Ph_grad + Velectron_mat
    ...
    for k1 in range (0,n*2):
        ...
        for l in range(n-2):
            if e_derivative[l]*e_derivative[l+1] < 0:
                ll=ll+1
        if ll==1 and k1 > 30 and k1 < n*2-30:
            Cb_E_address=k1
    ...
    for k2 in range (0,n*2):
        ...
        for l in range(n-2):
            if h_derivative[l]*h_derivative[l+1] < 0:
                lll=lll+1
        if lll==1 and k2 > 30 and k2 < n*2-30:
            Vb_E_address=k2
    ...
    if abs(float(Cb_energy-old_Cb_E)) < 0.002:
        break
    old_Cb_E=Cb_energy
