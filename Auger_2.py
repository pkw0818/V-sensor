...
mh1=0.6;    # CdTe me=0.1  mh=0.4
#me2=0.04*9.1e-31;   
#mh2=1*9.1e-31;
er1=9;           # CdSe: 9.3(WZ) 10.2(ZB)        # CdTe: 10.2(ZB)
er2=12;           # CdS: 8.9 or 7 or 6.3
Eg=2.0;
Cbo=2.0;           # CdSe qX: 4.6     Eg: 2.1
Vbo=2.0;             # CdTe qX: 4.1     Eg: 1.8
k=math.sqrt(21./2);           # K dot P matrix element
delta=1e-11;              # use for avoid divergence in V(q)

#Define Geometry
L=20e-9;                # Sim. length L >> QW (confined area)
Qw = 15e-9;
space=1e-10;
X = np.linspace(-L , L, 2*L/space+1)
n= X.size
region=[-L,-15e-9,-5e-9,15e-9,L]
indx=np.zeros(len(region))
for var in range(len(region)):
    indx[var]=(region[var]-region[0])/space

CB_value=[Cbo,0.5,0,Cbo]
VB_value=[Vbo,0,0.3,Vbo]
er_value=[1,er1,er1,1]
me_value=[me1,me1,me1,me1]
mh_value=[mh1,mh1,mh1,mh1]
...
for q in range(f0.size):
    xq=1j*f0[q]*delta
    Vq[q]=-expi(-xq)*e**(xq)-expi(xq)*e**(-xq)
Vq_prime=Vq*eq_x1_FT
Vq_prime=Vq_prime[f0.size/2]
...
dq=f0[1]-f0[0]
Vq_prime_integral=sum(Vq_prime[Vq_prime_integral_range_lower[0]:Vq_prime_integral_range_upper[0]+1])*dq
